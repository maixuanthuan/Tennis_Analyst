    ...
    def train(self, model, optimizer, epoch):
        ...
        for iter, batch in enumerate(train_progress_bar):
            ...
            train_progress_bar.set_description(
                'Train: Epoch: {}/{}. Iter: {}/{}. Loss: {}'.format(epoch + 1, self.num_epochs, iter + 1,
                                                                    self.max_iters, loss.item()))
        ...
        for iter, batch in enumerate(val_progress_bar):
            with torch.no_grad():
                ...
                val_progress_bar.set_description(
                    'Val: Epoch: {}/{}. Iter: {}/{}. Loss: {}'.format(epoch + 1, self.num_epochs, iter + 1,
                                                                      len(self.val_loader), loss.item()))
        return np.mean(losses)
